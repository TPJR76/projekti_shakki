...
käyttäjä voi pelata shakkia joko kaksinpelinä tai yksinpelinä.
"""

from tkinter import *
from teorialauta import Teorialauta, shakkilaudan_koordinaatit
from koulutus import arvioi_paras_siirto
...
class Shakkilauta:
    ...
    def kaksinpelin_vuorot(self):
        ...
        # Tarkistetaan voitto
        voittaja = self.__pelilauta.tarkista_voitto()
        if voittaja == "musta":
    ...
    def yksinpelin_vuorot(self):
        ...
        # Tarkistetaan voitto
        if self.__pelilauta.tarkista_voitto() == "valkoinen":
            # Valkoinen on voittanut pelin
            self.__tilateksti.configure(text="Peli on päättynyt, valkoinen "
                                             "voitti!")
            return
        if self.__pelilauta.tarkista_voitto() == "musta":
            # Musta on voittanut pelin
            self.__tilateksti.configure(text="Peli on päättynyt, "
                                             "musta voitti!")
            return
        ...
        if self.__pelilauta.siirtonumero() % 2 == 1:
            if self.__pelimuoto == 2:
                ...
                sijainti, siirto = arvioi_paras_siirto(self.__pelilauta,
                                                       "valkoinen")
    ...
    def aseta_nappulat_ja_tarkista_shakit(self):
        ...
        # Tarvitaan Teorialaudan nappulakohtaiset asemalistat sekä
        # yhteenlasketut asemat
        valkoisen_asemat, mustan_asemat = self.__pelilauta.asemat()

        for ruutu in valkoisen_asemat + mustan_asemat:
        ...
        # Tarkistetaan shakit
        shakitettu_valkoinen_kuningas, shakitettu_musta_kuningas = \
            self.__pelilauta.shakitetut_kuninkaat()
        # Merkataan shakissa oleva kuningas punaisella
        if shakitettu_musta_kuningas:
            self.nappi_koordinaatille(shakitettu_musta_kuningas).configure(
                bg="Red", activebackground="Red")
        if shakitettu_valkoinen_kuningas:
            self.nappi_koordinaatille(shakitettu_valkoinen_kuningas).configure(
                bg="Red", activebackground="Red")
    ...
    def aktivoi_oikean_puolen_napit(self):
        ...
        if self.__pelilauta.siirtonumero() % 2 == 1:
            oma_puoli, vast_puoli = self.__pelilauta.asemat()
        else:
            vast_puoli, oma_puoli = self.__pelilauta.asemat()

        for ruutu in vast_puoli:
    ...
    def nappi_koordinaatille(self, merkkijono):
        """
        Apufunktio, joka parittaa ruudun koordinaattiin käyttöliittymässä
        koordinaattia vastaavassa paikassa sijaitsevan napin.
